            ...
            {leads.map((l:any)=>(
              <div key={l.id} style={{ background:'#1e293b', border:'1px solid #334155', borderRadius:12, padding:'14px 16px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                  <div><div style={{ fontSize:13, fontWeight:600, color:'#e2e8f0' }}>{l.nombre}</div><div style={{ fontSize:11, color:'#475569', marginTop:2 }}>{l.ciudad||'—'} · {l.fuente||'—'}</div></div>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', paddingTop:8, borderTop:'1px solid #0f172a' }}>
                  <div><div style={{ fontSize:11, color:'#475569' }}>Interés</div><div style={{ fontSize:12, color:'#cbd5e1', fontWeight:500 }}>{l.vehiculo_interes}</div></div>
                  <div style={{ textAlign:'right' }}><div style={{ fontSize:11, color:'#475569' }}>Vendedor</div><div style={{ fontSize:11, color:'#94a3b8' }}>{l.vendedor_nombre}</div></div>
                </div>
                {l.proximo_contacto && <div style={{ marginTop:8, fontSize:11, color:'#fb923c' }}>⏰ {l.proximo_contacto}</div>}
              </div>
            ))}
