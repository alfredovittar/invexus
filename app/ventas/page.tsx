  ...
  const handleGuardar = async () => {
    ...
    if (form.inv_id) {
      const { error: errInv } = await supabase
        .from('inventario')
        .update({ estado: 'Vendido', fecha_venta: form.fecha || hoy })
        .eq('id', form.inv_id)
    ...
    if (gastosForm.length > 0 && form.inv_id) {
      const gastosInsert = gastosForm
        ...
        .map(g => ({
          inv_id: form.inv_id, venta_id: newId,
          empresa: form.empresa, tipo: g.tipo,
          descripcion: g.descripcion, monto_ars: g.monto_ars,
      ...
      if (!instrId) {
        const { data: newInstr } = await supabase
          ...
          .insert({
            ...
            numero_referencia: inst.numero_referencia,
            banco: inst.banco, monto_ars: inst.monto_ars,
          ...
          <div style={{ background: '#1e293b', border: '1px solid #3b82f6', borderRadius: 12, padding: 20, marginBottom: 16 }}>
              ...
              <select value={form.inv_id} onChange={e => setForm({ ...form, inv_id: e.target.value })} style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: '1px solid #334155', background: '#1e293b', color: '#e2e8f0', fontSize: 12 }}>
                ...
                {vehiculosParaVenta.map((v: any) => (
                  <option key={v.id} value={v.id}>
                    [{v.id}] {v.marca} {v.modelo} {v.version} — {v.color}{v.vin ? ` | VIN: ${v.vin}` : ''}{v.patente ? ` | Pat: ${v.patente}` : ''}{v.dias_stock ? ` (${v.dias_stock}d)` : ''}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 12 }}>
              {[['cobro_efectivo', 'Efectivo ARS'], ['cobro_transfer', 'Transferencia ARS'], ['cobro_pagare', 'Pagaré ARS'], ['cobro_pxp', 'Parte de pago ARS']].map(([k, l]) => (
                <div key={k}>
                  <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>{l}</label>
                  <input type="number" value={(form as any)[k]} onChange={e => setForm({ ...form, [k]: parseFloat(e.target.value) || 0 })} style={{ width: '100%', padding: '6px 10px', borderRadius: 8, border: '1px solid #334155', background: '#0f172a', color: '#e2e8f0', fontSize: 12 }} />
                </div>
              ))}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>USD</label>
                <input type="number" value={form.cobro_usd} onChange={e => setForm({ ...form, cobro_usd: parseFloat(e.target.value) || 0 })} style={{ width: '100%', padding: '6px 10px', borderRadius: 8, border: '1px solid #334155', background: '#0f172a', color: '#e2e8f0', fontSize: 12 }} />
              </div>
              <div>
                <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>TC pactado</label>
                <input type="number" value={form.cobro_usd_tc} onChange={e => setForm({ ...form, cobro_usd_tc: parseFloat(e.target.value) || tcBna })} style={{ width: '100%', padding: '6px 10px', borderRadius: 8, border: '1px solid #334155', background: '#0f172a', color: '#fb923c', fontSize: 12 }} />
              </div>
                ...
                <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 8, padding: 10, marginBottom: 10, maxHeight: 200, overflowY: 'auto' }}>
                  {instrumentosExistentes.filter(i => i.estado !== 'aplicado_total').length === 0
                    ...
                    : instrumentosExistentes.filter(i => i.estado !== 'aplicado_total').map(i => (
                        <div key={i.id} onClick={() => agregarInstrumentoExistente(i)}
                          style={{ padding: '6px 10px', borderRadius: 6, cursor: 'pointer', fontSize: 12, color: '#e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}
            ...
            <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 10, padding: '12px 14px', marginBottom: 12 }}>
              ...
              {gastosForm.length > 0 && (
                <div style={{ textAlign: 'right', fontSize: 12, color: '#fb923c', fontFamily: 'monospace', paddingTop: 6, borderTop: '1px solid #1e293b' }}>
                  Total gastos: {fmt(totalGastosForm)}
                </div>
              )}
            </div>
            ...
            <div style={{ background: diff > 100 ? 'rgba(239,68,68,.1)' : diff < -100 ? 'rgba(234,179,8,.1)' : 'rgba(34,197,94,.1)', border: `1px solid ${diff > 100 ? 'rgba(239,68,68,.3)' : diff < -100 ? 'rgba(234,179,8,.3)' : 'rgba(34,197,94,.3)'}`, borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: 12 }}>
              ...
              <span style={{ color: diff > 100 ? '#f87171' : diff < -100 ? '#facc15' : '#4ade80', fontWeight: 600 }}>
                {diff > 100 ? 'Faltan: ' + fmt(diff) : diff < -100 ? 'Exceso: ' + fmt(Math.abs(diff)) : 'Cobro completo ✓'}
              </span>
            </div>
                    ...
                    <React.Fragment key={v.id}>
                      ...
                      >
                        ...
                        <td style={{ padding: '8px 10px' }}>
                          <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 4, background: v.empresa === 'INVEXUS' ? 'rgba(96,165,250,.15)' : 'rgba(167,139,250,.15)', color: v.empresa === 'INVEXUS' ? '#60a5fa' : '#a78bfa', border: `1px solid ${v.empresa === 'INVEXUS' ? 'rgba(96,165,250,.3)' : 'rgba(167,139,250,.3)'}` }}>{v.empresa}</span>
                        </td>
                        ...
                        <td style={{ padding: '8px 10px' }}>
                          <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, fontWeight: 600, background: ec.bg, color: ec.color, border: `1px solid ${ec.border}` }}>{v.estado_cobro}</span>
                        </td>
                            ...
                            <div style={{ background: '#0f1f3d', borderTop: '1px solid #1e3a5f', padding: '20px 24px' }}>
                              {!isEditing ? (
                                ...
                                <div onClick={e => e.stopPropagation()}>
                                  ...
                                  <div style={{ display: 'flex', gap: 8 }}>
                                    <button onClick={e => { e.stopPropagation(); handleGuardarEdicion(v.id) }} style={{ padding: '6px 18px', borderRadius: 7, background: '#16a34a', color: 'white', border: 'none', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>Guardar cambios</button>
