      ...
      <div style={{padding:'20px 24px',maxWidth:1400,margin:'0 auto'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
          ...
          <div style={{background:'#1e293b',borderRadius:12,border:'1px solid #334155',padding:'16px 18px'}}>
            <div style={{fontSize:11,color:'#64748b',fontWeight:500,letterSpacing:'.06em',textTransform:'uppercase',marginBottom:14}}>Stock crítico (+60 días)</div>
            {criticos.length===0
              ?<div style={{color:'#475569',fontSize:13}}>Sin alertas activas ✓</div>
              :criticos.slice(0,6).map((s:any)=>(
                <div key={s.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid #0f172a'}}>
                  <div>
                    ...
                    <div style={{fontSize:11,color:'#64748b'}}>{s.color} · {s.dias_stock} días</div>
                  </div>
                  <span style={{fontSize:10,fontWeight:600,padding:'2px 8px',borderRadius:4,background:s.dias_stock>=90?'rgba(239,68,68,.15)':'rgba(249,115,22,.15)',color:s.dias_stock>=90?'#f87171':'#fb923c',border:`1px solid ${s.dias_stock>=90?'rgba(239,68,68,.3)':'rgba(249,115,22,.3)'}`}}>
                    {s.dias_stock>=90?'CRÍTICO':'ALERTA'}
                  </span>
                </div>
              ))
            }
